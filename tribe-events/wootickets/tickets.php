		...
		foreach ( $tickets as $ticket ) :
			...
			if ( $ticket->date_in_range() ) {
				...
				if ( 0 !== $available ) {
					...
					$max_quantity = $product->is_sold_individually() ? 1 : $max_quantity;
					$available    = $ticket->available();
					$start_date = new DateTime($post->EventStartDate);
					$new_date = new DateTime('2019-04-30');
					$new_2020_date = new DateTime('2020-04-30');
					$curr_user_id = get_current_user_id();

					$members_ticket = get_post_meta($ticket->ID, '_members_only', true);
					$show_ticket = true;

					if($members_ticket === 'yes' && $start_date >= $new_2020_date) {
						if($curr_user_id != 0) {
							$memberships = wc_memberships_get_user_memberships($curr_user_id, array(
								'status' => array('active')
							));
							$membership = !empty($memberships) ? $memberships[0] : '';

							if($membership->plan->slug == 'corporate-21-22' || $membership->plan->slug == 'corporate-plus-22-23' || $membership->plan->slug == 'corporate-22-23' || $membership->plan->slug == 'corporate-plus' || $membership->plan->slug == 'corporate') {
								$max_membership_quantity = 9999;
							} else if($membership->plan->slug == 'sole-trader-22-23' || $membership->plan->slug == 'student-22-23' || $membership->plan->slug == 'sole-trader-21-22' || $membership->plan->slug == 'student-21-22' ) {
								$max_membership_quantity = 1;
							} else if($membership->plan->slug == 'committee') {
								$max_membership_quantity = 9999;
							} else if($membership->plan->slug == 'corporate-20-21') {
								$max_membership_quantity = 3;
							} else {
								$show_ticket = false;
								$max_membership_quantity = 0;
							};


							if ($max_quantity > $max_membership_quantity) {
								$max_tickets = $max_membership_quantity;
							} else if($max_quantity < 1) {
								$max_tickets = $max_membership_quantity;
							} else {
								$max_tickets = $max_quantity;
							}
						} else {
							$show_ticket = false;
							$max_tickets = $max_quantity;
						}
					} else if($members_ticket === 'yes' && $start_date >= $new_date) {
						if($curr_user_id != 0) {
							$memberships = wc_memberships_get_user_memberships($curr_user_id);
							$membership = !empty($memberships) ? $memberships[0] : '';
							if($membership->plan->slug == 'corporate-20-21') {
								$max_membership_quantity = 3;
							} else if($membership->plan->slug == 'corporate-plus-20-21') {
								$max_membership_quantity = 9999;
							} else if($membership->plan->slug == 'individual-20-21' || $membership->plan->slug == 'student' || $membership->plan->slug == 'retired') {
								$max_membership_quantity = 1;
							} else if($membership->plan->slug == 'committee') {
								$max_membership_quantity = 9999;
							} else {
						...
						if($members_ticket === 'yes' && $curr_user_id != 0) {
							$memberships = wc_memberships_get_user_active_memberships($curr_user_id);
							$membership = !empty($memberships) ? $memberships[0] : '';
							if($membership->plan->slug == 'corporate') {
								$max_membership_quantity = 3;
							} else if($membership->plan->slug == 'corporate-plus') {
								$max_membership_quantity = 9999;
							} else if($membership->plan->slug == 'individual' || $membership->plan->slug == 'student' || $membership->plan->slug == 'retired') {
								$max_membership_quantity = 1;
							} else if($membership->plan->slug == 'committee') {
								$max_membership_quantity = 9999;
							} else {
					...
					// var_dump($max_membership_quantity);
					...
					if($show_ticket) {
						...
						echo 'Only available to CE Glos Members';
					}
...
if ( $is_there_any_product_to_sell ) {
	...
	// @todo remove safeguard in 4.3 or later
	if ( $unavailability_messaging ) {
		$unavailability_message = $this->maybe_show_tickets_unavailable_message( $tickets );
